...
int main() {
    ...
    for(int i = 0;i < 2;i++){
        v[i]=(final[i]-initial[i])/t;  //x, y velocity (const)
        dp[i]=(final[i]-initial[i])/N; //x, y displacement (const)
    } 
    ...
    for(int k = 0;k < N;k++){ //Iterate through each step in N steps
        ...
        //Theta 3
        theta[1]=asin((yb-(L1*sin(theta[0])))/L2)-theta[0];
        ...
        //Joint rates (theta_dot = Jinv*theta)
        mat3x3_vec_mult(theta_dot, Jinv, theta);
